function [cost] = evalObjects(image, objects, prevObjects)
  ...
  for i = 1:nobjects
    ...
    minxi = max([1 ceil((obj.pos(1) - obj.width / 2) / xscale)]);
    maxxi = min([size(hue, 2) ceil((obj.pos(1) + obj.width / 2) / xscale)]);
    minyi = max([1 ceil((obj.pos(2) - obj.height / 2) / yscale)]);
    maxyi = min([size(hue, 1) ceil((obj.pos(2) + obj.height / 2) / yscale)]);
